...
describe("scalar insert edge cases", () => {
  ...
  it("preserves NULL for missing rows and NULL projections instead of applying a JS default", async () => {
    const nullableDefault = createTable({
      id: "nullable_default",
      schema: z.object({ value: z.string().nullable().default("fallback") }),
    });
  ...
  it("rolls back the entire bulk insert if a later row violates a constraint", async () => {
    await expect(
      Promise.resolve(
        db.insertMany(target, [
          { id: "duplicate", value: projection() },
          { id: "duplicate", value: "literal" },
        ]),
      ),
    ).rejects.toThrow();
